...
class Exporter:
    ...
    def _generate_python_code(self, actions: List[Action]) -> str:
        ...
        if self.include_window_setup and self.target_window_title:
            ...
            lines.append(f'        target_hwnd = win32gui.FindWindow(None, "{self.target_window_title}")')
            lines.append("        if target_hwnd:")
            lines.append("            win32gui.ShowWindow(target_hwnd, win32con.SW_RESTORE)")
            lines.append("            win32gui.SetForegroundWindow(target_hwnd)")
            lines.append("            time.sleep(0.5)")
            lines.append("            rect = win32gui.GetWindowRect(target_hwnd)")
            ...
            lines.append("    ")
            lines.append("    def get_window_offset():")
            lines.append('        """Get current window offset in real-time."""')
            lines.append("        nonlocal window_x, window_y, target_hwnd")
            lines.append("        if not target_hwnd:")
            lines.append("            return window_x, window_y")
            lines.append("        try:")
            lines.append("            import win32gui")
            lines.append("            if not win32gui.IsWindow(target_hwnd):")
            lines.append("                print('Warning: Target window no longer exists')")
            lines.append("                return window_x, window_y")
            lines.append("            rect = win32gui.GetWindowRect(target_hwnd)")
            lines.append("            window_x, window_y = rect[0], rect[1]")
            lines.append("            return window_x, window_y")
            lines.append("        except Exception as e:")
            lines.append("            print(f'Warning: Failed to get window offset: {{e}}')")
            lines.append("            return window_x, window_y")
            lines.append("    ")
            lines.append("    def validate_window():")
            lines.append('        """Validate target window exists."""')
            lines.append("        nonlocal target_hwnd")
            lines.append("        if not target_hwnd:")
            lines.append("            return True")
            lines.append("        try:")
            lines.append("            import win32gui")
            lines.append("            return win32gui.IsWindow(target_hwnd)")
            lines.append("        except:")
            lines.append("            return False")
            lines.append("    ")
            lines.append("    def activate_window():")
            lines.append('        """Bring target window to front."""')
            lines.append("        nonlocal target_hwnd")
            lines.append("        if not target_hwnd:")
            lines.append("            return")
            lines.append("        try:")
            lines.append("            import win32gui")
            lines.append("            import win32con")
            lines.append("            if win32gui.IsWindow(target_hwnd):")
            lines.append("                win32gui.ShowWindow(target_hwnd, win32con.SW_RESTORE)")
            lines.append("                win32gui.SetForegroundWindow(target_hwnd)")
            lines.append("                time.sleep(0.05)")
            lines.append("        except Exception as e:")
            lines.append("            print(f'Warning: Failed to activate window: {{e}}')")
            lines.append("    ")
            ...
            lines.append("    ")
            lines.append("    def get_window_offset():")
            lines.append('        """Get current window offset."""')
            lines.append("        return window_x, window_y")
            lines.append("    ")
    ...
    def _action_to_code(self, action: Action) -> str:
        ...
        if action.action_type == ActionType.MOUSE_CLICK:
            ...
            x, y = action.params.get('x', 0), action.params.get('y', 0)
            code_lines.append("activate_window()")
            code_lines.append("wx, wy = get_window_offset()")
            if action.action_type == ActionType.MOUSE_MOVE_RELATIVE:
                duration = action.params.get('duration', 0.0)
                code_lines.append(f"pyautogui.moveTo(x=wx + {x}, y=wy + {y}, duration={duration})")
            else:
                code_lines.append(f"pyautogui.click(x=wx + {x}, y=wy + {y})")
