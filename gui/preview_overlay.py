...
class PreviewOverlay(QWidget):
    ...
    def show_image_match(self, image_path: str, confidence: float = 0.9, region: Optional[Tuple[int, int, int, int]] = None):
        self._preview_type = 'image'
        self._preview_data = {'image_path': image_path, 'confidence': confidence, 'region': region}
        self._start_preview()
    ...
    def _draw_image_match(self, painter: QPainter):
        ...
        match_region = self._preview_data.get('region')
        ...
        try:
            ...
            try:
                ...
                if match_region:
                    locate_kwargs['region'] = match_region
            ...
            if location:
                ...
                try:
                    screenshot = pyautogui.screenshot(region=match_region) if match_region else pyautogui.screenshot()
                    screenshot_cv = cv2.cvtColor(np.array(screenshot), cv2.COLOR_RGB2BGR)
                    template = cv2.imread(image_path)
                    if template is not None:
                        result = cv2.matchTemplate(screenshot_cv, template, cv2.TM_CCOEFF_NORMED)
